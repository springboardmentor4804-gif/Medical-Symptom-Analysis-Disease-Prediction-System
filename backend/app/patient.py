...
def predict_patient_disease(
    ...
    # Fetch recent symptom severities for triage
    severity_map = {}
    try:
        cursor.execute(
            """
    ...
    # Fetch patient profile details for triage
    vitals_data = {}
    try:
        cursor.execute(
            """
    ...
    try:
        ...
        if risk_row:
            latest_risk_score = float(risk_row[0])
    except Exception:
        pass

    # Evaluate 4-Tier Severity Triage & Red Flags
    triage_result = evaluate_triage(
        symptoms=request.symptoms,
        symptom_severities=severity_map,
        vitals=vitals_data,
        predicted_disease=predicted_disease,
        risk_score_percentage=latest_risk_score,
    )
