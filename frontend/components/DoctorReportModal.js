...
export default function DoctorReportModal({ isOpen, onClose, reportData, loading }) {
  ...
  const handleDownloadPdf = async () => {
    if (!reportData?.patient?.id) return;
    setDownloading(true);
    try {
      const patientId = reportData.patient.id;
      const patientName = (reportData.patient.name || 'Patient').replace(/\s+/g, '_');
      await api.downloadFile(
        `/doctor/patients/${patientId}/report/pdf`,
        `Doctor_Clinical_Report_${patientName}.pdf`
      );
    } catch (e) {
      console.error("Doctor PDF download error:", e);
      window.print();
    } finally {
        ...
        <div className="flex items-center justify-between px-6 py-4 border-b border-slate-200 dark:border-slate-800 bg-slate-50 dark:bg-slate-950/80 print:hidden">
          ...
          <div className="flex items-center gap-2 sm:gap-3">
            <button
              ...
              className="inline-flex items-center gap-2 px-3.5 py-2 rounded-xl bg-teal-600 hover:bg-teal-700 text-white text-xs font-semibold shadow-sm transition-all disabled:opacity-50"
            >
              {downloading ? (
                <>
        ...
        <div className="p-6 sm:p-10 max-h-[80vh] overflow-y-auto print:max-h-none print:overflow-visible print:p-8">
          {loading ? (
            ...
            <div id="printable-doctor-report" className="space-y-8 text-slate-900 dark:text-slate-100 print:text-black">
              ...
              <div className="p-4 rounded-2xl bg-amber-500/10 dark:bg-amber-950/70 border-2 border-amber-400 dark:border-amber-600 text-amber-950 dark:text-amber-100 print:bg-amber-50 print:border-amber-500 print:text-black space-y-1.5 shadow-sm">
                <div className="flex flex-wrap items-center justify-between gap-2 border-b pb-1.5 border-amber-300/60 dark:border-amber-800/80 print:border-amber-400">
                  <div className="flex items-center gap-2">
                    ...
                    <h4 className="text-xs font-black uppercase tracking-wider text-amber-900 dark:text-amber-200 print:text-black">
                      Clinical Advisory & Model Disclaimer
                    </h4>
                  </div>
                  <span className="text-[10px] font-extrabold uppercase px-2 py-0.5 rounded-full bg-amber-200 dark:bg-amber-900/80 text-amber-900 dark:text-amber-200 print:border print:border-amber-500">
                ...
                <p className="text-xs leading-relaxed text-amber-950 dark:text-amber-100 font-medium print:text-black">
                  <strong className="font-extrabold">CLINICAL NOTICE:</strong> This consultation summary contains AI model disease likelihood estimations derived from logged symptom data. <strong className="font-bold underline">These metrics are statistical model predictions intended solely for physician decision support and do NOT replace official clinical laboratory diagnostics or independent physician evaluation.</strong> Always verify findings through formal medical examination and licensed physician judgment.
                </p>
              </div>
