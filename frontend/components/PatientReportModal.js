  ...
  const handleDownloadPdf = async () => {
    ...
    try {
      await api.downloadFile(
        '/patients/me/report',
        `MedAssist_AI_Health_Report_${reportData?.patient?.id || 'Patient'}.pdf`
      );
    } catch (e) {
      console.error("PDF download error, falling back to window print:", e);
      window.print();
    } finally {
      ...
      {isOpen && (
        ...
        <div className="flex items-center justify-between px-6 py-4 border-b border-slate-200 dark:border-slate-800 bg-slate-50 dark:bg-slate-950/80 print:hidden">
          ...
          <div className="flex items-center gap-2">
            <button
              ...
              className="inline-flex items-center gap-2 px-3.5 py-2 rounded-xl bg-emerald-600 hover:bg-emerald-700 text-white text-xs font-semibold shadow-sm transition-all disabled:opacity-50"
            >
              {downloading ? (
                <>
            ...
            <button
              ...
              className="inline-flex items-center gap-1.5 px-3 py-2 rounded-xl bg-slate-200 dark:bg-slate-800 hover:bg-slate-300 dark:hover:bg-slate-700 text-slate-700 dark:text-slate-200 text-xs font-semibold transition-all disabled:opacity-50"
            >
              <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth="2">
        ...
        <div className="p-6 sm:p-10 max-h-[80vh] overflow-y-auto print:max-h-none print:overflow-visible print:p-8">
          {loading ? (
            ...
            <div id="printable-report" className="space-y-8 text-slate-900 dark:text-slate-100 print:text-black">
              ...
              {/* PROMINENT OFFICIAL MEDICAL DISCLAIMER BANNER */}
              <div className="p-4 sm:p-5 rounded-2xl bg-amber-500/10 dark:bg-amber-950/70 border-2 border-amber-400 dark:border-amber-600 text-amber-950 dark:text-amber-100 print:bg-amber-50 print:border-amber-500 print:text-black space-y-2 shadow-sm">
                <div className="flex flex-wrap items-center justify-between gap-2 border-b pb-2 border-amber-300/60 dark:border-amber-800/80 print:border-amber-400">
                  <div className="flex items-center gap-2">
                    <svg className="w-5 h-5 text-amber-600 dark:text-amber-400 shrink-0 print:text-amber-700" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth="2.5">
                      <path strokeLinecap="round" strokeLinejoin="round" d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-3L13.732 4c-.77-1.333-2.694-1.333-3.464 0L3.34 16c-.77 1.333.192 3 1.732 3z" />
                    </svg>
                    <h4 className="text-xs sm:text-sm font-black uppercase tracking-wider text-amber-900 dark:text-amber-200 print:text-black">
                      Official Clinical Advisory & Medical Disclaimer
                    </h4>
                  </div>
                  <span className="text-[10px] font-extrabold uppercase px-2.5 py-0.5 rounded-full bg-amber-200 dark:bg-amber-900/80 text-amber-900 dark:text-amber-200 print:border print:border-amber-500">
                    AI Prediction Model • Not a Diagnosis
                  </span>
                </div>
                <p className="text-xs leading-relaxed text-amber-950 dark:text-amber-100 font-medium print:text-black">
                  <strong className="font-extrabold">IMPORTANT NOTICE:</strong> This report presents automated algorithmic disease risk predictions generated by an artificial intelligence model based on submitted symptoms. <strong className="font-bold underline decoration-amber-500">This document is strictly an AI prediction and DOES NOT constitute a medical diagnosis, certified lab result, or physician's opinion.</strong> AI predictions must never replace clinical evaluation or diagnostic testing. <strong className="font-black text-amber-900 dark:text-amber-200">Always consult a licensed physician or qualified healthcare provider for clinical evaluation, diagnosis, and medical advice.</strong> In case of emergency, contact local emergency medical services immediately.
                </p>
              </div>
        ...
        <div className="px-6 py-4 border-t border-slate-200 dark:border-slate-800 bg-slate-50 dark:bg-slate-950/80 flex items-center justify-between print:hidden">
          <span className="text-xs text-slate-500 dark:text-slate-400">
            MedAssist AI Clinical Health Record & Risk Report
          </span>
          <div className="flex items-center gap-2">
            <button
              onClick={handleDownloadPdf}
