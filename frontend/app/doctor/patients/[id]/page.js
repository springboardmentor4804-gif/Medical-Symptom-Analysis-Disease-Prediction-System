...
export default function DoctorPatientHistoryPage() {
  ...
  const handleDownloadPdfReport = async () => {
    ...
    try {
      const nameStr = (patient?.name || `Patient_${patientId}`).replace(/\s+/g, '_');
    ...
    <ProtectedRoute allowedRoles={['doctor']}>
      <div className="flex-1 px-4 sm:px-6 py-8 bg-slate-50 dark:bg-slate-950 text-slate-900 dark:text-slate-100">
        <div className="max-w-5xl mx-auto space-y-8">
          ...
          <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 pb-4 border-b border-slate-200 dark:border-slate-800">
            ...
            <div className="flex items-center gap-2.5 flex-wrap">
              <button
                ...
                className="px-4 py-2.5 rounded-xl bg-slate-900 hover:bg-slate-800 dark:bg-slate-800 dark:hover:bg-slate-700 text-white text-xs font-semibold shadow-md transition-all flex items-center gap-2 disabled:opacity-50"
              >
                {pdfDownloading ? (
                  <>
          ...
          {/* CLINICAL CASE REVIEW ADVISORY BANNER */}
          <div className="p-4 rounded-2xl bg-amber-500/10 dark:bg-amber-950/70 border-2 border-amber-400 dark:border-amber-600 text-amber-950 dark:text-amber-100 space-y-1.5 shadow-sm">
            <div className="flex flex-wrap items-center justify-between gap-2 border-b pb-1.5 border-amber-300/60 dark:border-amber-800/80">
              <div className="flex items-center gap-2">
                ...
                <h4 className="text-xs font-black uppercase tracking-wider text-amber-900 dark:text-amber-200">
                  Attending Physician Clinical Case Advisory
                </h4>
              </div>
            ...
            <p className="text-xs leading-relaxed text-amber-950 dark:text-amber-100 font-medium">
              <strong className="font-extrabold">CLINICAL REVIEW NOTICE:</strong> Automated triage scores, organ system risk scores, and ML condition matches are provided strictly as supportive decision intelligence. <strong className="font-bold underline">Final clinical diagnosis, official prescriptions, and treatment directives remain the sole medical responsibility of the attending licensed physician.</strong>
            </p>
          </div>
