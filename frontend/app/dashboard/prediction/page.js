            ...
            <div className="lg:col-span-7 space-y-6">
              ...
              {result ? (
                <>
                  ...
                  {/* OFFICIAL MEDICAL DISCLAIMER BANNER */}
                  <div className="p-4 rounded-2xl bg-amber-500/10 dark:bg-amber-950/70 border-2 border-amber-400 dark:border-amber-600 text-amber-950 dark:text-amber-100 space-y-1.5 shadow-xs">
                    <div className="flex items-center justify-between gap-2">
                      ...
                      <span className="text-[10px] font-extrabold uppercase px-2 py-0.5 rounded-full bg-amber-200 dark:bg-amber-900/80 text-amber-900 dark:text-amber-200">
                        Consult Physician
                      </span>
                    </div>
                    <p className="text-xs leading-relaxed text-amber-950 dark:text-amber-100 font-medium">
                      This analysis is an <strong className="font-extrabold">automated statistical AI model prediction</strong> based on input data. <strong className="font-bold underline">It does NOT constitute a medical diagnosis or physician evaluation.</strong> Always consult a licensed physician or healthcare professional for clinical evaluation and accurate medical advice.
                    </p>
                  </div>
                  ...
                  <ScaleIn className={`bg-white dark:bg-slate-900 border rounded-2xl p-6 shadow-sm relative overflow-hidden ${
                    ...
                    {/* Action Buttons: Direct PDF Download & Full Report Preview */}
                    <div className="mt-4 pt-4 border-t border-slate-100 dark:border-slate-800 flex flex-wrap items-center justify-end gap-3">
                      <button
                        type="button"
                        onClick={async () => {
                          try {
                            await api.downloadFile('/patients/me/report', `MedAssist_AI_Prediction_Report_${profile?.id || 'Patient'}.pdf`);
                          } catch (err) {
                            console.error("Direct PDF download error:", err);
                            handleGenerateReportWithPredictions();
                          }
                        }}
